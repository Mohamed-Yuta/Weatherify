...
public class WeatherService {
    ...
    private final String geoUrl2="http://api.openweathermap.org/geo/1.0/zip?zip={zip code}&appid={API key}";
    ...
    public Coord getCoordByZipCode(String zipCode){
        return webClient.build()
                .get()
                .uri(geoUrl,zipCode,key)
                .retrieve()
    ...
    public WeatherData getWeatherByZipCode(String zipCode){
        Coord coord = getCoordByZipCode(zipCode);
        return webClient.build().get()
                .uri(url,coord.getLat(),coord.getLon(),key)
                .retrieve()
                .bodyToMono(WeatherData.class)
